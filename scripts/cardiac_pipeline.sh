...
echo "[1/6] Loading cardiac datasets (standardization + profiling)"
python3 "$ROOT_DIR/scripts/data/load_cardiac.py"
echo ""

echo "[2/6] Preprocessing datasets (split + scale + fairness profiles)"
python3 "$ROOT_DIR/scripts/data/preprocess_cardiac.py"
echo ""

echo "[3/6] Training baseline model(s)"
python3 "$ROOT_DIR/scripts/models/train_baseline.py"
echo ""

echo "[4/6] Assessing post-prediction fairness"
python3 "$ROOT_DIR/scripts/fairness/assess_predictions.py"
echo ""

# Experiment stage
if [[ "$RUN_AGE_BINNING" == "true" ]]; then
    echo "[5/6] Age binning strategies analysis"
    python3 "$ROOT_DIR/scripts/experiments/run_age_binning_analysis.py" \
        --config "$AGE_BINNING_CONFIG"
    echo ""
else
    echo "[5/6] Age binning analysis SKIPPED"
fi

if [[ "$RUN_MITIGATION" == "true" ]]; then
    echo "[6/6] Mitigation techniques comparison"
    python3 "$ROOT_DIR/scripts/experiments/run_mitigation_comparison.py" \
        --config "$MITIGATION_CONFIG"
    echo ""
else
    echo "[6/6] Mitigation comparison SKIPPED"
fi
...
echo "  - Baseline models:    $ROOT_DIR/experiments/cardiac/baseline"
echo "  - Fairness results:   $ROOT_DIR/results/cardiac"
if [[ "$RUN_AGE_BINNING" == "true" ]]; then
    echo "  - Age binning:        $ROOT_DIR/results/experiments/age_binning"
fi
if [[ "$RUN_MITIGATION" == "true" ]]; then
    echo "  - Mitigation:         $ROOT_DIR/results/experiments/mitigation"
fi
echo ""
echo "Usage examples:"
...
echo "  RUN_AGE_BINNING=false RUN_MITIGATION=false bash scripts/cardiac_pipeline.sh"
echo ""

...
def decode_sensitive_attributes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    age_mapping = {
        -2.0: '<40', -1.5: '<40',  # Handles some variance from scaling
        -1.0: '40-49', -0.5: '40-49',
        0.0: '50-59', 0.5: '50-59',
        1.0: '60-69', 1.5: '60-69',
        2.0: '70+', 2.5: '70+'
...
def assess_dataset_fairness(
    ...
    exclude_cols = ['y_true', 'y_pred', 'y_proba', 'threshold', 'age_group', 
                    'sex', 'confidence', 'near_threshold', 'age_group_cat', 'sex_cat']
    ...
    for attr, attr_metrics in metrics.get('group_fairness', {}).items():
        ...
        for metric_name, metric_data in attr_metrics.items():
            ...
            if metric_name == 'demographic_parity':
                ...
                logging.info(f"    {is_fair} Equalized Odds: TPR diff={tpr_diff:.3f}, FPR diff={fpr_diff:.3f}")
...
def main():
    ...
    metrics_calculator = FairnessMetrics(
        sensitive_attributes=['age_group_cat', 'sex_cat']
    )
